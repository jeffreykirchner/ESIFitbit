...
class Session_day_subject_actvity(models.Model):
    ...
    def pullFitbitActvities(self):
        ...
        if immune_activity_minutes !=-1 and settings.DEBUG == False:
            #activites
    ...
    def getCSVResponse(self, writer):
        ...
        writer.writerow([f'{self.session_day.session.title}', self.session_day.period_number, self.session_day.session.parameterset.getBlock(self.session_day.period_number).block_number,
                         self.session_day.getDateStr(),self.session_subject.group_number,self.session_subject.id_number, self.session_subject.student_id, self.heart_activity_minutes,
